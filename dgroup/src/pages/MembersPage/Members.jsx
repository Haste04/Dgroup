...
export default function Members() {
  ...
  return (
    <div className="p-6 md:p-10 max-w-7xl mx-auto animate-fade-in flex flex-col items-start md:items-center md:justify-center min-h-[80vh] overflow-x-hidden">
      
      {/* Header stays left on mobile, center on PC */}
      <header className="mb-8 md:mb-12 text-center md:text-center w-full">
      ...
      {/* CHANGE: Changed justify-start back to justify-center for mobile centering */}
      <div className="relative w-full flex items-center justify-center gap-0 md:gap-8">
        
        {/* Left Arrow: Still hidden on mobile */}
        ...
        {/* CHANGE: Removed the -ml-10px to keep it centered */}
        <div className="w-full max-w-[100vw] md:max-w-[920px]">
          <Swiper
            effect={'coverflow'}
            grabCursor={true}
            // CHANGE: Set back to true for mobile centering
            centeredSlides={true} 
            slidesPerView={'auto'} 
            initialSlide={0}
            coverflowEffect={{
          ...
          >
            {members.map((person) => (
              <SwiperSlide key={person.id} className="max-w-[280px] md:max-w-[380px]">
                <div 
        ...
        {/* Right Arrow: Also hidden on mobile */}
      ...
      {/* Modal remains centered (PC & Mobile) as it is a fixed overlay */}
      {selectedMember && (
