...
export default function Members() {
  ...
  return (
    <div className="p-4 md:p-10 max-w-7xl mx-auto animate-fade-in flex flex-col items-center justify-center min-h-[80vh] overflow-x-hidden">
      ...
      <div className="relative w-full flex items-center justify-center gap-0 md:gap-8">
        ...
        <div className="w-full max-w-[100vw] md:max-w-[920px]">
          <Swiper
            ...
            initialSlide={0}
            coverflowEffect={{
              rotate: 30, // Reduced rotation for better mobile viewing
              stretch: 0, 
              depth: 150, // Reduced depth for mobile
              modifier: 1,
              slideShadows: false,
            }}
            navigation={{
          ...
          >
            {members.map((person) => (
              /* Adjusted card width for mobile (260px) vs desktop (380px) */
      ...
      {/* Your Modal Code Below remains the same as our previous version */}
      {selectedMember && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md animate-fade-in">
          <div className="bg-[#0A0C10] border border-white/20 rounded-3xl max-w-2xl w-full relative shadow-[0_0_50px_rgba(0,0,0,0.5)] overflow-y-auto md:overflow-hidden flex flex-col md:flex-row max-h-[90vh] md:max-h-none">
            ...
            <div className="w-full md:w-3/5 p-6 md:p-8 relative">
              ...
              <span className={`font-bold uppercase text-[10px] tracking-[0.2em] bg-clip-text text-transparent bg-gradient-to-r ${selectedMember.color}`}>{selectedMember.role}</span>
              ...
              <div className="space-y-5">
                <div>
                  ...
                  <p className={`italic text-base bg-clip-text text-transparent bg-gradient-to-r ${selectedMember.color} brightness-125`}>"{selectedMember.verse}"</p>
                </div>
                <div>
                  <h4 className="text-[10px] font-bold text-slate-500 uppercase mb-1">Brief Bio</h4>
                  <p className="text-slate-300 text-sm">{selectedMember.bio || "No bio added yet."}</p>
                </div>
              </div>
            </div>
          </div>
